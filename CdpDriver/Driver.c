...
NTSTATUS CdpAddDevice(_In_ PDRIVER_OBJECT DriverObject, _In_ PDEVICE_OBJECT PhysicalDeviceObject)
{
	...
	KeInitializeEvent(&DeviceExtension->JournalBackendReadyEvent,
		NotificationEvent, FALSE);
	InterlockedExchange(&DeviceExtension->JournalBackendReady, 0);
	InterlockedExchange(&DeviceExtension->VolumeOnline, 0);
	...
	KeInitializeEvent(&DeviceExtension->VolumeIoDrainedEvent,
		NotificationEvent, TRUE);
	InterlockedExchange(&DeviceExtension->VolumeIoAccepting, 0);
	InterlockedExchange(&DeviceExtension->VolumeIoOutstanding, 0);
	...
	Cdp_LOG("attached kind=volume filter=%p lower=%p pdo=%p\n",
		FilterDeviceObject,
