...
设置还原点时，驱动把目标时间视图通过磁盘直写器物化到源分区，删除目标 Record 及之前的历史（完整 RR 直接回收、边界 RR 的旧数据 Record tombstone），然后在 Superblock 保存持久标记。边界 RR 的分支结构 Header 保留，以确保其中的后续数据在下次挂载仍可恢复所属分支。重启自动发现看到该标记后：
