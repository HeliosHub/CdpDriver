...
def delete_all_files(directory: str):
    """删除指定目录下的所有文件，保留文件夹结构"""
    if not os.path.exists(directory):
        print(f"错误：目录 '{directory}' 不存在")
        return
        
    print(f"--- 开始清空目录: {directory} ---")
    try:
...
if __name__ == "__main__":
    ...
    #------------------------------------------
        ...
        print("--- 设置还原点 ---")
        run_program([cdp_tool, "set-restore-point", protected_disk_guid, password, str(t1)])

        for i in range(14):
            print(f"这是第 {i+1} 次循环，当前 i 的值是: {i}")
            print("--- 写文件 目标1GB ---")
            run_program(["python", gen_file_script, "-s", "1GB", "-o", protected_disk_path + "\\"])
            delete_all_files(protected_disk_path + "\\")
        print("--- 写文件 目标1GB ---")
        run_program(["python", gen_file_script, "-s", "1GB", "-o", protected_disk_path + "\\"])
        ...
        print("--- 睡眠30秒继续执行 ---")
        time.sleep(30)
        run_program([cdp_tool, "checkpoints", protected_disk_guid, password])
    #------------------------------------------
    finally:
        print("--- 关闭保护 ---")
